...
App.controller('applicationCompController', ['$scope', '$controller', '$location', 'appConst', 'AppBizCom', 'logicCom', 'AppCom', 'logFileSend', 'slsDelete',
    function ($scope, $controller, $location, appConst, AppBizCom, logicCom, AppCom, logFileSend, slsDelete) {
        ...
        /** 画面ID. */
        var PAGE_THIS_ID: string = 'G1270-01';
        /** 送信成功画面ID. */
        var PAGE_SUCCESS_ID: string = 'G1270-02';
        /** 送信成功画面名. */
        var PAGE_SUCCESS_NAME: string = 'applicationCompSendSuccess';
        /** 送信失敗画面ID. */
        var PAGE_FAILED_ID: string = 'G1270-03';
        /** 送信失敗画面名. */
        var PAGE_FAILED_NAME: string = 'applicationCompSendFailed';
        ...
        $scope.sendInit = function (): void {
            ...
            new Promise((resolve: any, reject: any) => {
                ...
                searchRrcptNoTubn(
                    ...
                    }, (error: { msg: string, param: any }) => {
                        // 失敗
                        return reject(error);
                    }
                );

            }).then((rcptNoTubn: number) => {
                return new Promise((resolve: any, reject: any) => {

                    try {
                        var applyInfo: any = AppBizCom.DataHolder.getNotifInfo();   // 申込データ（事務手続き）
                        var eFormInfo: any = AppBizCom.DataHolder.getEFormInfo();   // 申込データ（電子帳票）

                        if (applyInfo) {
                            // SUBIF008-SENDの作成（事務手続き）
                            var subifSend: any = createSubif008Send(rcptNoTubn);
                            return resolve({
                                subifSend: subifSend,
                                type: '2'
                            });
                            ...
                            var subifSend: any = createSubif009Send(rcptNoTubn);
                            return resolve({
                                subifSend: subifSend,
                                type: '3'
                            });
            ...
            }).then((result: { subifSend: any, type: string }) => {
                return new Promise((resolve: any) => {

                    // 申込データSLS保存
                    saveSlsApplyInfo(result.type, result.subifSend,
                        (key: string) => {
                            // 成功
                            return resolve({
                        ...
                        }, (error: { msg: string, param: any }, key: string) => {
                            // 失敗
            ...
            }).then((result: { subifSend: any, type: string, key: string }) => {
                return new Promise((resolve: any) => {
            ...
            }).then((error: { msg: string, param: any }) => {
                return new Promise((resolve: any) => {
            ...
            }).catch((error: { msg: string, param: any }) => {
         ...
         * @returns void
         */
        var sendLog = function (successCallback: () => void): void {
         ...
         * @returns void
         */
        var saveSlsApplyInfo = function (type: string, subifSend: any, successCallback: (key: string) => void, errorCallback: (error: { msg: string, param: any }, key: string) => void): void {

            var time: string = AppCom.Date.getCurrentTimeMillis();
            var key: string = time + type;
         ...
         * @returns void
         */
        var deleteSlsApplyInfo = function (key: string, successCallback: () => void, errorCallback: (error: { msg: string, param: any }) => void): void {
         ...
         */
        var createSubif009Send = function (rcptNoTubn: number): any {

            var eFormInfo: any = AppBizCom.DataHolder.getEFormInfo();       // 申込データ（電子帳票）
            var imageData: any = AppBizCom.DataHolder.getImageData();       // 画像データ

            var EIGYOIN_JOHO: any = eFormInfo.EIGYOIN_JOHO || {};           // 営業員情報
            var MOSKM_HSK: any = eFormInfo.MOSKM_HSK || {};                 // 申し込み補足情報
            var CHOHYO: any = eFormInfo.CHOHYO || {};                       // 電子帳票
            var UKE_JOHO: any = eFormInfo.UKE_JOHO || {};                   // 受付場所情報
            var KYAK_JOHO: any = eFormInfo.KYAK_JOHO || {};                 // 受付場所情報

            var serviceTime: any = AppBizCom.DataHolder.getServiceTime();   // サービス時間情報
            var GYOMU_DATE: string = serviceTime.GYOMU_DATE;                // 業務日付
         ...
         */
        var createSubif008Send = function (rcptNoTubn: number): any {

            // 共通領域データ取得
            var applyInfo: any = AppBizCom.DataHolder.getNotifInfo();               // 申込データ（事務手続き）
            var offerMynumberData: any = AppBizCom.DataHolder.getPersonInfo();      // 申込データ（特定個人情報）
            var imageData: any = AppBizCom.DataHolder.getImageData();               // 画像データ
            var apageJumpFlg: any = AppBizCom.DataHolder.getFlowControlFlg();       // 画面遷移制御フラグ情報            
            ...
            // 申込データ
            var EIGYOIN_JOHO: any = applyInfo.EIGYOIN_JOHO || {};                   // 営業員情報
            var KYAK_JOHO: any = applyInfo.KYAK_JOHO || {};                         // お客さま情報
            var MOSKM_HSK: any = applyInfo.MOSKM_HSK || {};                         // 申し込み補足情報
            var UKE_JOHO: any = applyInfo.UKE_JOHO || {};                           // 受付場所情報
            var MNSYSEIRY_JOHO: any = applyInfo.MNSYSEIRY_JOHO || {};               // 番号確認書類情報
            var HONIN_KAKNIN_SY_JOHO: any = applyInfo.HONIN_KAKNIN_SY_JOHO || {};   // 本人確認書類情報
            var JIMU_JOHO: any = applyInfo.JIMU_JOHO || {};                         // 事務手続き情報
            var PIN_JOHO: any = applyInfo.PIN_JOHO || {};                           // 暗証番号情報
            var EIGYOIN_KAKNIN_JOHO: any = applyInfo.EIGYOIN_KAKNIN_JOHO || {};     // 営業員確認情報
            var KAKNIN_SY_CHK_JOHO: any = applyInfo.KAKNIN_SY_CHK_JOHO || {};       // 確認書類チェック情報

            // 画面遷移制御フラグ情報
            var INPUT_FLG_CONTROL: any = apageJumpFlg.INPUT_FLG_CONTROL || {};      // 入力画面遷移制御
            ...
            if (MNSYSEIRY_JOHO.MNSYSEIRY_K && offerMynumberData.MYNO) {
                var myno: string = MNSYSEIRY_JOHO.MNSYSEIRY_K + offerMynumberData.MYNO;
            ...
            if (HONIN_KAKNIN_SY_JOHO.HONIN_KAKNIN_SY_K_1) {
                var codeMstData: any = AppBizCom.MstData.getCodeMstDataByCd('HONIN_KAKNIN_SY', HONIN_KAKNIN_SY_JOHO.HONIN_KAKNIN_SY_K_1);
                var stm4: string = codeMstData ? codeMstData.STM4 : '';
            ...
            if (HONIN_KAKNIN_SY_JOHO.HONIN_KAKNIN_SY_K_2) {
                var codeMstData: any = AppBizCom.MstData.getCodeMstDataByCd('HONIN_KAKNIN_SY', HONIN_KAKNIN_SY_JOHO.HONIN_KAKNIN_SY_K_2);
                var stm4: string = codeMstData ? codeMstData.STM4 : '';
            ...
            if (JIMU_JOHO.NENSY_JSY) {
                var codeMstData: any = AppBizCom.MstData.getCodeMstDataByCd('TDFKN_C', JIMU_JOHO.NENSY_JSY);
                var stm2: string = codeMstData ? codeMstData.STM2 : '';
            ...
            if (JIMU_JOHO.TKTEI_KOZA_NENSY_JSY) {
                var codeMstData: any = AppBizCom.MstData.getCodeMstDataByCd('TDFKN_C', JIMU_JOHO.TKTEI_KOZA_NENSY_JSY);
                var stm2: string = codeMstData ? codeMstData.STM2 : '';
            ...
            if (JIMU_JOHO.KOZA && JIMU_JOHO.KOZA.length) {
                for (var i: number = 0, length: number = JIMU_JOHO.KOZA.length; i < length; i++) {
                    var koza: any = JIMU_JOHO.KOZA[i];
            ...
            // 振込先口座-削除
                ...
                for (var i: number = 0, length: number = INPUT_FLG_CONTROL.KOZA_DEL_F.length; i < length; i++) {
                    var kozaDelF: string = INPUT_FLG_CONTROL.KOZA_DEL_F[i];
                    ...
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL01 = kozaDelF;
                    //} else if (i === 1) {
                    //    // 変更後情報.振込先口座-登録No_削除02
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL02 = kozaDelF;
                    //} else if (i === 2) {
                    //    // 変更後情報.振込先口座-登録No_削除03
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL03 = kozaDelF;
                    //} else if (i === 3) {
                    //    // 変更後情報.振込先口座-登録No_削除04
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL04 = kozaDelF;
                    //} else if (i === 4) {
                    //    // 変更後情報.振込先口座-登録No_削除05
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL05 = kozaDelF;
                    //} else if (i === 5) {
                    //    // 変更後情報.振込先口座-登録No_削除06
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL06 = kozaDelF;
                    //} else if (i === 6) {
                    //    // 変更後情報.振込先口座-登録No_削除07
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL07 = kozaDelF;
                    //} else if (i === 7) {
                    //    // 変更後情報.振込先口座-登録No_削除08
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL08 = kozaDelF;
                    //} else if (i === 8) {
                    //    // 変更後情報.振込先口座-登録No_削除09
                    //    SUBIF.HNKATO.KOZA_TORK_NO_DEL09 = kozaDelF;
                    //}
                    // 2020/11/19 インシデント対応 削除完了
                }
            }
         ...
         * @returns void
         */
        var searchRrcptNoTubn = function (successCallback: (rcptNoTubn: number) => void, errorCallback: (error: { msg: string, param: any }) => void) {

            // DBオブジェクト
            var logDb: any = AppCom.ClientDb.open();

            new Promise((resolve: any, reject: any) => {

                // 受付NO通番取得
                var sql: string = 'SELECT RCPT_NO_TUBN FROM KR_RCPT_NO_KNR';

                AppCom.ClientDb.execSQL(logDb, sql, [],
                    (resultSet: any) => {
                        // 成功
                ...
                return new Promise((resolve: any, reject: any) => {
                    ...
                    // 受付NO通番カウントアップ
                    var sql: string = 'UPDATE KR_RCPT_NO_KNR SET RCPT_NO_TUBN = ?';
                    var increment: number = rcptNoTubn + 1;
         ...
         * @returns void
         */
        var sendSubif = function (type: string, formData: any, successCallback: () => void, errorCallback: (error: { msg: string, param: any }) => void) {

            var SUBIF_CONST: any = type == '2' ? appConst.SUBIF008 : appConst.SUBIF009;

            // REDOSサーバに申込データ送信
            AppCom.Http.post(SUBIF_CONST.PATH, formData,
                (subifRecv: any, status: number) => {
         ...
         * @returns void
         */
        var sendToServer = function (subifSend: any, type: string, key: string, successCallback: () => void, errorCallback: (error: { msg: string, param: any }) => void): void {

            new Promise((resolve: any, reject: any) => {

                try {
                    // マルチパート変換
                    var formData: any = conversionMultipart(subifSend);
         ...
         * @returns void
         */
        var sendApplyInfo = function (key: string, successCallback: () => void, errorCallback: (error: { msg: string, param: any }) => void): void {

            new Promise((resolve: any, reject: any) => {

                // SLSデータを取得
                logicCom.getSLSDatas([key],
                    (data: any) => {
                        // 成功
                        if (!data || !data.length) {
                            return reject({
            ...
            }).then((subifInfo: { subifSend: any, type: string }) => {
                return new Promise((resolve: any, reject: any) => {

                    try {
                        // マルチパート変換
                        var formData: any = conversionMultipart(subifInfo.subifSend);
                        return resolve({
                            formData: formData,
                            type: subifInfo.type
                        });
            ...
            }).then((subifInfo: { formData: any, type: string }) => {
                return new Promise((resolve: any, reject: any) => {
         ...
         */
        var conversionMultipart = function (subIf: any): any {
