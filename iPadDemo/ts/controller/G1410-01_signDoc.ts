...
App.controller('signDocController', ['$scope', '$timeout', '$controller', 'AppBizCom', 'logicCom', 'AppComDevice', 'AppComDate',
    function ($scope, $timeout, $controller, AppBizCom, logicCom, AppComDevice, AppComDate) {
        ...
        /** 画面ID. */
        var BEHIND_PAGE_ID: string = 'G1400-01';
        var PAGE_ID: string = 'G1410-01';
        var PAGE_ID_SIGN: string = 'G1410-02';
        var PAGE_ID_COMP: string = 'G1410-03';
        var PAGE_ID_SENT: string = 'G1270-01';
        var BASE64_HEAD: string = 'data:image/jpeg;base64,';

        /** ボタン. */
        var SHOME_BTN_NAME: string = '署名する';
        var ALLDEL_BTN_NAME: string = 'すべて削除';
        var RETURN_BTN_NAME: string = '戻る';
        var FIX_BTN_NAME: string = '確定';
        ...
        $scope.init = function () {
            ...
            img.onload = function () {     // 画像ファイル読込後の処理
                ...
                canvas_main = <HTMLCanvasElement>document.getElementById("canvas_main");
                var ctx: any = canvas_main.getContext('2d');
         ...
         *  テキスト情報を取得
         */
        var drawText = function (ctx: CanvasRenderingContext2D) {
            // 店部課コード
            var miseCode: string = eFormInfo.KYAK_JOHO.MISE_C.substr(1, 3);
            // 客コード
            var kyakCode: string = eFormInfo.KYAK_JOHO.KYAK_CIF_C;
            // メールアドレスアカウント名
            var mailCode: string = eFormInfo.CHOHYO.MAIL_ACCOUNT;
            // 帳票IDを取得する
            var choHyoId = eFormInfo.CHOHYO.CHOHYO_ID;
            // ドメイン名入力フラグを取得する
            var mailFlag: string = eFormInfo.CHOHYO.MAIL_DOMAIN_FLAG;
            // メールアドレスドメインコード
            var codeCd: string = eFormInfo.CHOHYO.MAIL_DOMAIN_C;
            // ドメイン名を取得
            if (choHyoId != null) {
            ...
            switch ($scope.docCode) {
                case "870":
                    ...
                    // 店部課コードの入力
                    for (var i = 0; i < 3; i++) {
                        var miseStr: string = miseCode.substring(i, i + 1);
                        ctx.fillText(miseStr, 756 + (i + 1) * 64, 520);
                    }
                    // 客コードの入力
                    for (var j = 0; j < 7; j++) {
                        var kyakStr: string = kyakCode.substring(j, j + 1);
                    ...
                    // 指定メールアドレスの出力
                    var mailStr: string = mailCode + '@' + $scope.domainValue;
                    var returnLength = 29;
        ...
        */
        ...
        $scope.modalConfirmBtnClick = function () {
            ...
            signImg.onload = function () {
                ...
                // 社員ID
                var eigyoinStr: string = eFormInfo.EIGYOIN_JOHO.PROPER_C;

                // 申込データに事前に最新の位置情報を仮設定
                var locationData = getObject(AppBizCom.DataHolder.getLocation());
                var offerMailData: any = AppBizCom.DataHolder.getEFormInfo();
            ...
            var setImageData = function () {
                ...
                var canvas_main = <HTMLCanvasElement>document.getElementById("canvas_main");
                var ctx = canvas_main.getContext('2d');
                // 書面データの取得
         ...
         * @return void
         */
        var setIdoAndKeido = function (idoStr: string, keidoStr: string): void {
            // 共通領域の位置情報に設定
            var positionJoho = {
                IDO: idoStr,
                KEIDO: keidoStr
        ...
        var setImgMesssage = function (ctx, signImg, eigyoinStr, idoStr, keidoStr) {
            switch ($scope.docCode) {
                case "870":
                    // 署名の出力
                    var tempCanvas1: HTMLCanvasElement = document.createElement('canvas');
                    ...
                    var tempCanvas2: HTMLCanvasElement = document.createElement('canvas');
        ...
        // データの位置を作成する
        var drawCheckBox = function (ctx: CanvasRenderingContext2D, xPos: number, yPos: number) {
            ctx.strokeStyle = '#000';
            ctx.lineWidth = 1;
            ctx.beginPath();
