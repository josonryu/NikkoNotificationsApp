...
App.controller('splashController', ['$scope', '$location', '$controller', '$interval', '$timeout', 'appConst', 'AppCom', 'AppBizCom', 'logicCom', 'dbInfoConst', 'slsDelete', 'logFileSend', 'APP_MODE', 'APL_KBN', 'APL_NAME', 'CURRENT_CONFIG', 'TEST_CONFIG',
function ($scope, $location, $controller, $interval, $timeout, appConst, AppCom, AppBizCom, logicCom, dbInfoConst, slsDelete, logFileSend, APP_MODE, APL_KBN, APL_NAME, CURRENT_CONFIG, TEST_CONFIG) {
    // 共通部分制御を継承
    ...
    var MAST_FILE = {
        ...
        DB: {
            ...
            TARGET_ROOT_DIR: cordova.file.applicationStorageDirectory + 'Library/',
            // コピー先アプリDBファイル格納ディレクトリ
            TARGET_DIR: cordova.file.applicationStorageDirectory + 'Library/LocalDatabase/',
            // コピー先アプリDB格納ディレクトリ作成
        ...
        IMG: {
            ...
            TARGET_ROOT_DIR: cordova.file.applicationStorageDirectory + 'Documents/',
            // 画像ファイル格納ディレクトリ(imgフォルダ格納先)
            TARGET_IMG_DIR: cordova.file.applicationStorageDirectory + 'Documents/NoCloud/appBizFile/',
            // 画像ファイル格納ディレクトリ
            TARGET_DIR: cordova.file.applicationStorageDirectory + 'Documents/NoCloud/appBizFile/img/',
            // コピー先ディレクトリ作成
    ...
    /** 画面. */
    var PAGE_ID_MAIN: string = 'G1030-01';
    var PAGE_ID_NEXT: string = 'G1040-01';
    var PAGE_NM_NEXT: string = 'homeMenu';
    ...
    var ERR_OS_VERSION_WARN = {
        ...
        CONTENTS: '', // バージョンチェック時に設定
    };

    var clientDb: any;
    var responseWaitTimer: any;
    var cameraInterval: any;
    ...
    $scope.init = function () {
        ...
        new Promise((resolve, reject) => {
          ...
          logDbFileOperation(
            ...
            // 期限切れ申込データの削除
            slsDelete.deleteExpiredSlsData(
                (keys: Array<string>) => {
                    // 成功
                    if (keys && keys.length) {
     ...
     * 経路チェック
     */
    var checkRoute = function(successCallBack: any, errorCallBack: any) {

        // 既に起動された状態で起動イベントを検知させる
        (<any>window).handleOpenURL = function(url: string) {
            localStorage.setItem('launchURL', url);
            location.href = (<any>window).rootURL;
        }

        var url: string = localStorage.getItem('launchURL');
        localStorage.removeItem('launchURL');
        var filePath: string = cordova.file.applicationStorageDirectory  + 'Library/Preferences/';
        var fileName: string = 'appMode.txt';
        var attribute: any = {};
        ...
        new Promise((resolve, reject) => {
            ...
            // URLスキームパラメータ形式チェック
            var pattern: any = /^[a-z]+:\/\/\?[a-zA-Z]+\=[0-9]{7}\&[a-zA-Z]+\=[0-9]{4}\&[a-zA-Z]+\=[0-9]{4}\&[a-zA-Z]+\=.*$/g;
            var isFormat: Array<string> = url.match(pattern);
            if (!isFormat) {
                // URLスキームの形式が不正
            ...
            var params: Array<string> = param.split('&');
            var paramCnt: number = params.length;
            ...
            // モードファイルの存在を確認
            logicCom.existsFile(filePath + fileName, (isExists: boolean) => {
                resolve(isExists);
            });

        }).then((isExistFile) => { return new Promise((resolve, reject) => {

            var isCreateFile: boolean = false;

            // 既にモードファイルが存在する場合は作成しない
            if (isExistFile) return resolve(isCreateFile);
        ...
        })}).then((isCreateFile) => { return new Promise((resolve, reject) => {
            // モードファイルを作成した場合は読み取らない
            if (isCreateFile) return resolve(attribute.appMode);

            // モードファイルの読み取り
            logicCom.readFileTxtData(filePath + fileName,
                (appMode: string) => {
                    // 成功
        ...
        })}).then((appMode) => { return new Promise((resolve, reject) => {
            ...
            // ログイン者データの設定
            var loginInfo: any = {
                PROPER_C         　: attribute.syainID,  // 社員ID
                UKETSUKE_MISE_C  　: attribute.tenbukaC, // 受付者店部課コード
     ...
     */
    var checkTermStatus = function(successCallBack: any, errorCallBack: any) {
        ...
        new Promise((resolve, reject) => {
            ...
            // サーバ通信確認
            AppCom.Http.post(appConst.SUBIF001.PATH, {}, 
                (data: any, status: any) => {
                    // 成功
                    return resolve();
                    ...
                    // 失敗
                    var result: any = { data: data, status: status };
        ...
        // SLS件数取得（電子帳票）
        })}).then((sumLen) => { return new Promise((resolve, reject) => {
        ...
        // バッジ件数設定
        })}).then((len: number) => { return new Promise((resolve, reject) => {
        ...
        // GPS状態確認
            ...
            AppCom.Device.isGpsAvailable((available: any) => {
                return resolve(available);
            });
        
        // GPS許可要求
        })}).then((available) => { return new Promise((resolve, reject) => {
        ...
        // カメラ状態確認
            ...
            AppCom.Device.isCameraAvailable(
                (result: any) => {
                    // 成功
     ...
     */
    async function loginTimeAndVersionCheck(successCallBack: any, errorCallBack: any) {
        ...
        // ログイン可能時間・バージョンチェック
        }).then((SUBIF002) => { return new Promise((resolve, reject) => {

                AppCom.Http.post(appConst.SUBIF002.PATH, SUBIF002,
                    (data: any, status: any) => {
        ...
        // サービス時間情報の設定
        })}).then((data: any) => { return new Promise((resolve, reject) => {
            ...
            switch (Number(data.RESULT_CODE)) {
                ...
                case appConst.SUBIF002.RESULT_CODE.OFFLINE:
                    // オフライン中
                    ERR_LOGIN_TIME.CONTENTS = AppBizCom.Msg.getMsg('KKAP-CM000-12E',
                        [APL_NAME, formatTime('H:MM', data.SERVICE_START_TIME), formatTime('H:MM', data.LOGIN_END_TIME)]);
        ...
        // サービス時間情報を業務共通領域に保存
            ...
            var serviceTimeJoho = {
                SRVC_KS_ZKK    : formatTime('HH:MM:SS', data.SERVICE_START_TIME), // サービス開始時刻
                LOGIN_SYRY_ZKK : formatTime('HH:MM:SS', data.LOGIN_END_TIME),     // ログイン終了時刻
                SRVC_SYRY_ZKK  : formatTime('HH:MM:SS', data.SERVICE_END_TIME),   // サービス終了時刻
                GYOMU_DATE     : data.TODAY_BUSINESS_DAY.replace(/-/g, ''),       // 業務日付
            };
            AppBizCom.DataHolder.setServiceTime(serviceTimeJoho);

            // サービス時間外エラーメッセージ作成
            ERR_SERVICE_TIME.CONTENTS = AppBizCom.Msg.getMsg('KKAP-CM000-14E',
                [APL_NAME, formatTime('H:MM', data.SERVICE_START_TIME), formatTime('H:MM', data.SERVICE_END_TIME)]);
     ...
     * @param {callback} errorCallBack   - 失敗時のコールバック関数
     */
    var masterDataUpdate = function(successCallBack: any, errorCallBack: any) {
        ...
        new Promise((resolve, reject) => {
            ...
            // SQLを実行
            var clientDb: any = AppCom.ClientDb.open();
            AppCom.ClientDb.execSQL(clientDb, dbInfoConst.MAST_DATA.SEARCH_SQL, [], 
                (result: any) => {
                    // 成功
                    return resolve(result)
        ...
        }).then((result: any) => { return new Promise((resolve, reject) => {
            ...
            responseWaitTimer = $timeout(function() {
                responseWaitTimer = undefined;
                var error: any = { data: 'timeout', status: -1 };
                return reject([ERR_NETWORK, 'マスタデータ更新HTTP通信エラー', error]);
            }, 300000);

            // マスタデータ更新要求(サーバ側)
            AppCom.Http.post(appConst.SUBIF003.PATH, httpParam, 
                (data: any, status: any) => {
                    ...
                    if (angular.isDefined(responseWaitTimer)) {
                        ...
                        responseWaitTimer = undefined;
                        // システムエラー
                        var error: any = { data: data, status: status };
                        return reject([ERR_NETWORK, 'マスタデータ更新HTTP通信エラー', error]);
                    }
                }
            );
     ...
     * @param {callback} errorCallBack   - 失敗時のコールバック関数
     */
    var dbFileOperation = function(successCallBack: any, errorCallBack: any) {

        var isExistsEndFile: boolean = true;
        ...
        // 利用領域のエンドファイル存在確認
            ...
            logicCom.existsFile(MAST_FILE.DB.TARGET_DIR + MAST_FILE.END_FILE_NAME,
                (isExists: any) => {
        ...
        // エンドファイルを読み取り（利用領域）
        })}).then((isExists: any) => { return new Promise((resolve, reject) => {
        
            // 利用領域のエンドファイルが存在しない場合は読み取らない
            if (!isExists) return resolve();

            logicCom.readFileTxtData(MAST_FILE.DB.TARGET_DIR + MAST_FILE.END_FILE_NAME,
                (versionUse: any) => {
                    // 成功
        ...
        // エンドファイルを読み取り（配布領域）
        })}).then((versionUse) => { return new Promise((resolve, reject) => {

            var isCopy: boolean = true;

            // 利用領域のエンドファイルを読み取っていない場合は、配布領域も読み取らない
            if (!versionUse) return resolve(isCopy);

            logicCom.readFileTxtData(MAST_FILE.DB.SRC_DIR + MAST_FILE.END_FILE_NAME,
                (versionDistribute: any) => {
        ...
        })}).then((isCopy: any) => { return new Promise((resolve, reject) => {

            // アプリ初回インストール、またはアプリアップデート且つDBのバージョンに変更がない場合は実施しない
            if (!isExistsEndFile || (isExistsEndFile && !isCopy)) return resolve(isCopy);
        ...
        // データベースクローズ
            ...
            if (!clientDb) return resolve(isCopy);
        ...
        // エンドファイルコピー
            ...
            if (!isCopy) return resolve(isCopy);
        ...
        // DBファイルコピー
            ...
            logicCom.fileCopy(MAST_FILE.DB.SRC_DIR + MAST_FILE.DB.FILE_NAME, MAST_FILE.DB.TARGET_DIR, MAST_FILE.DB.FILE_NAME,
                () => {
                    // 成功
     ...
     * @param {callback} errorCallBack - 失敗時のコールバック関数
     */
    var imageFileOperation = function(successCallBack: any, errorCallBack: any) {
        ...
        // 利用領域のエンドファイル存在確認
            ...
            logicCom.existsFile(MAST_FILE.IMG.TARGET_DIR + MAST_FILE.END_FILE_NAME,
                (isExists: any) => {
                    return resolve(isExists);
                }
            );
        ...
        // エンドファイルを読み取り（利用領域）
            ...
            logicCom.readFileTxtData(MAST_FILE.IMG.TARGET_DIR + MAST_FILE.END_FILE_NAME,
                (versionUse: any) => {
                    // 成功
                    return resolve(versionUse);
        ...
        // エンドファイルを読み取り（配布領域）
            ...
            logicCom.readFileTxtData(MAST_FILE.IMG.SRC_DIR + MAST_FILE.END_FILE_NAME,
                (versionDistribute: any) => {
        ...
        // 画像ディレクトリ削除
        })}).then((isCopy) => { return new Promise((resolve, reject) => {
     ...
     * @param errorCallBack   - 失敗時のコールバック関数
     */
    var resetReceptionNumber = function(successCallBack: any, errorCallBack: any) {

        // 業務日付
        var serviceTime: any = AppBizCom.DataHolder.getServiceTime();
        var GYOMU_DATE: string = serviceTime.GYOMU_DATE;
     ...
     * @param errorCallBack   - 失敗時のコールバック関数
     */
    var selectRcptNo = function(successCallBack: any, errorCallBack: any) {
        ...
        // SQL実行
        AppCom.ClientDb.execSQL(clientDb, sql, [],
            (result: any) => {
                // 成功
     ...
     * @param errorCallBack   - 失敗時のコールバック関数
     */
    var updateRcptNo = function(successCallBack: any, errorCallBack: any) {
     ...
     */
    var formatTime = function(format: string, time: string) {
        var result: string =
            format.replace('HH', time.substr(0,2))
     ...
     * @param {callback} errorCallBack   - 失敗時のコールバック関数
     */
    var logDbFileOperation = function(successCallBack: any, errorCallBack: any) {

        var isExistsEndFile: boolean = false;
        ...
        // 利用領域のログエンドファイル存在確認
            ...
            logicCom.existsFile(MAST_FILE.DB.TARGET_DIR + MAST_FILE.LOG.END_FILE_NAME,
                (isExists: any) => {
                    isExistsEndFile = isExists;
                    return resolve(isExists);
                }
        ...
        // ログエンドファイルを読み取り（利用領域）
            ...
            logicCom.readFileTxtData(MAST_FILE.DB.TARGET_DIR + MAST_FILE.LOG.END_FILE_NAME,
                (versionUse: any) => {
                    // 成功
                    return resolve(versionUse);
        ...
        // ログエンドファイルを読み取り（配布領域）
            ...
            logicCom.readFileTxtData(MAST_FILE.DB.SRC_DIR + MAST_FILE.LOG.END_FILE_NAME,
                (versionDistribute: any) => {
        ...
        // ログDB削除
            ...
            if (!isExistsEndFile || (isExistsEndFile && !isCopy)) return resolve();

            (<any>window).sqlitePlugin.deleteDatabase({name: 'aplLog.db', location: 'default'},
                () => {
                    // 成功
                    return resolve();
