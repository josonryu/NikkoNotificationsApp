...
App.controller('ocrIdResultController', ['$scope', '$controller', 'appConst', 'AppBizCom', 'logicCom', '$anchorScroll', '$timeout',
    function ($scope, $controller, appConst, AppBizCom, logicCom, $anchorScroll, $timeout) {
        // エラーモーダル用共通コントローラ
        $controller('errorInfoModalCtrl', {$scope: $scope});

        // 画面ID定義
        var MAIN_PAGE_ID: string = 'G1190-01';
        ...
        var personDocInfos: any = AppBizCom.MstData.getCodeMstDataByCd('HONIN_KAKNIN_SY', selectedInfo.HONIN_KAKNIN_SY_K_1);
        ...
        $scope.takeRetry = target => {
            ...
            // OCR対象書面の場合
            if ((personDocInfos.CD == '01' && target == 'front') || // 個人番号カード（表面）
                (personDocInfos.CD == '02' && target == 'front') || // 運転免許証（表面）
                (personDocInfos.CD == '04' && target == 'front') || // 在留カード（表面）
                (personDocInfos.CD == '05' && target == 'front')){  // 特別永住者証明書（表面）
        ...
        $scope.nextBtnClick = (): void => {
            ...
            var callback = function () {
                ...
                // 本人確認書類2種類目の表面画像存在する場合、2つ目の本人確認書類撮影結果表示する
            ...
            if ($scope.isAutoFillImage) {
                ...
                if (!AppBizCom.InputCheck.isEmpty(frontImageBase64) && $scope.showAutoFillFrontImage) {
                    var dest: string = 'HONIN_KAKNIN_SY1_GAZO1'; // セット先の共通領域項目名
                    var org: string = BASE64_HEAD + imageData[dest];
                ...
                if (!AppBizCom.InputCheck.isEmpty(backImageBase64) && $scope.showAutoFillBackImage) {
                    var dest: string = 'HONIN_KAKNIN_SY1_GAZO2'; // セット先の共通領域項目名
                    var org: string = BASE64_HEAD + imageData[dest];
