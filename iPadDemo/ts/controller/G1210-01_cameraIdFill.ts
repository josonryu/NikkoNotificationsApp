...
declare var AppBizImageFillMask: any;
...
App.controller('cameraIdFillController', ['$scope', 'appConst', 'AppBizCom', 'logicCom', '$anchorScroll', '$timeout', '$controller',
    function ($scope, appConst, AppBizCom, logicCom, $anchorScroll, $timeout, $controller) {
        var BACK_SCREEN_ID: string = '';
        var SCREEN_ID_01: string = 'G1210-01';
        var SCREEN_ID_02: string = 'G1210-02';
        var SCREEN_ID_03: string = 'G1210-03';
        ...
        // 共通領域定義 申込みデータ取得
        var comDataApplyInfo: any = AppBizCom.DataHolder.getNotifInfo();
        // 共通領域定義 撮影画像データ取得
        var comDataPhotoData: any = AppBizCom.DataHolder.getImageData();

        // 本人確認書類2種類目選択フラグ
        var docChoice2Flg: boolean = false;

        // 遷移元画面取得
        var prevPath: string = AppBizCom.DataHolder.getPrevRoutePath();
        ...
        var init = (): void => {
            ...
            if(prevPath == 'ocrIdResult') {
                commKey = {
                    // 塗りつぶし前画像
                    'frontOrgImageKey': 'HONIN_KAKNIN_SY1_GAZO1',        // 本人確認書類(1種類目)[表]
                    // 【裏面】
                    // 塗りつぶし前画像
                    'backOrgImageKey': 'HONIN_KAKNIN_SY1_GAZO2',        // 本人確認書類(1種類目)[裏]
                    // 【3枚目】
                    // 塗りつぶし前画像
                ...
                commKey = {
                    // 塗りつぶし前画像
                    'frontOrgImageKey': 'HONIN_KAKNIN_SY2_GAZO1',        // 本人確認書類(2種類目)[表]
                    // 【裏面】
                    // 塗りつぶし前画像
                    'backOrgImageKey': 'HONIN_KAKNIN_SY2_GAZO2',        // 本人確認書類(2種類目)[裏]
                    // 【3枚目】
                    // 塗りつぶし前画像
           ...
           // 【表面】
            ...
            // 画像塗りつぶし描画クラス生成
            var fillImageFromtClass: any = new AppBizImageFillMask('canvas', "canvasModalScroll", "canvasOrg", "canvasForStLine");
            ...
            var fillImageBackClass: any = new AppBizImageFillMask('canvasBack', "canvasBackModalScroll", "canvasBackOrg", "canvasBackForStLine");
            // Canvasタップ時イベントコールバック関数を登録
            fillImageBackClass.registCanvasOnTapEvent($scope.tooltipDestroy);
            $scope.fillImageBackClass = fillImageBackClass;
            var fillImageThirdClass: any = new AppBizImageFillMask('canvasThird', "canvasThirdModalScroll", "canvasThirdOrg", "canvasThirdForStLine");
            // Canvasタップ時イベントコールバック関数を登録
        ...
        $scope.maskBtnClick = target => {
            ...
            var orgImgBase64String: string = $scope.idDocFrontimg;
            // 裏面塗りつぶしの場合の変数初期化
            if (target === 'back') {
                $scope.fillImageClass = null;
                ...
                // logBtnName = LOG_BTN_ITEM1.ITEM_5;
            }
            // ３枚目塗りつぶしの場合の変数初期化
            if (target === 'third') {
                $scope.fillImageClass = null;
            ...
            var filledImageData: string = $scope[key];
            // Canvasに画像セット
            $scope.fillImageClass.setImageData(orgImgBase64String, (): void => {
                $scope.$applyAsync();
            }, filledImageData);
        ...
        $scope.btnNextClick = (): void => {
            ...
            // 塗りつぶし済み[表]画像をセット
            var frontImageBase64: string = typeof($scope.frontImageFill) != 'undefined'? $scope.frontImageFill.replace(regExpDelete, '') : appConst.NO_INPUT_VALUE;  
            // 塗りつぶし済み[裏]画像をセット
            var backImageBase64: string = typeof ($scope.backImageFill) != 'undefined' ? $scope.backImageFill.replace(regExpDelete, '') : appConst.NO_INPUT_VALUE;
           // 塗りつぶし済み[3枚目]画像をセット
            var thirdImageBase64: string = typeof ($scope.thirdImageFill) != 'undefined' ? $scope.thirdImageFill.replace(regExpDelete, '') : appConst.NO_INPUT_VALUE;
            // 申し込みデータ共通領域セット
            var comDataPhotoData: any = AppBizCom.DataHolder.getImageData();
            var imgList: any = [];
            // 本人確認書類[表]
            ...
            if (!AppBizCom.InputCheck.isEmpty(frontImageBase64)) {
                var dest: string = commKey.frontOrgImageKey; // セット先の共通領域項目名
                var org: string = BASE64_HEAD + comDataPhotoData[dest];
                var filled: string = BASE64_HEAD_PNG + frontImageBase64;
                imgList.push({org: org, filled: filled, dest: dest});
            }
            // 本人確認書類[裏]
            // 塗りつぶし済み画像がある場合は、撮影画像と結合させる。
            if (!AppBizCom.InputCheck.isEmpty(backImageBase64)) {
                var dest: string = commKey.backOrgImageKey; // セット先の共通領域項目名
                var org: string = BASE64_HEAD + comDataPhotoData[dest];
                var filled: string = BASE64_HEAD_PNG + backImageBase64;
                imgList.push({org: org, filled: filled, dest: dest});
            }
            // 本人確認書類[3枚目]
            // 塗りつぶし済み画像がある場合は、撮影画像と結合させる。
            if (!AppBizCom.InputCheck.isEmpty(thirdImageBase64)) {
                var dest: string = commKey.thirdOrgImageKey; // セット先の共通領域項目名
                var org: string = BASE64_HEAD + comDataPhotoData[dest];
                var filled: string = BASE64_HEAD_PNG + thirdImageBase64;
                imgList.push({org: org, filled: filled, dest: dest});
            }
        ...
        $scope.closeBtnClick = (): void => {
            ...
            scrollUnlock();
            var base64Data: any = $scope.fillImageClass.getImageData();
