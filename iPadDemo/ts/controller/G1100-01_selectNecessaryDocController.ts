...
App.controller('selectNecessaryDocController', ['$scope','AppBizCom','logicCom',
    function ($scope,AppBizCom,logicCom) {
        ...
        // 画面ID定義
        var MAIN_PAGE_ID: string = 'G1100-01'; // 番号確認書類のご説明・ご選択画面画面
        var PRE_PAGE_ID: string = 'G1090-01';  // 確認書類撮影開始画面
        var NEXT_PAGE_ID: string = 'G1110-01'; // 個人番号入力画面

        // 画面PATH定義
        var NEXT_PAGE_PATH: string = 'mynumberInput';
        ...
        // 共通領域保存処理
        var setNotifInfo = (numDoc: string): void => {
        ...
        // イベント：番号確認書類の選択時
        $scope.checkAreaClick = (selectDocument): void => {
            // ダブルタップ対応
            if (stopBtnEventFLG) {
                return;
            } else {
        ...
        $scope.btnBackClick = (): void => {
            ...
            var from: string = MAIN_PAGE_ID;
            var to: string = PRE_PAGE_ID;
            var btnName: string = '戻る';
