...
class HandWritePanel {
    ...
    isComposing = false; // 入力未確定状態フラグ
    isInputTouched = false; // false:タッチする要素が入力欄ではない true:タッチする要素が入力欄である
    touchedElement = undefined; // bodyのtouchstartイベントが発火時の対象DOM要素のid属性
    modeChanging: boolean = false; // 手書部品初期化状態フラグ
    keyboardHeightPlus: number = 0;
    currentType: string = '';
    attrDispName: string ='data-dispName';
    attrMaxChar: string ='maxlength';
    attrType: string ='data-type';
    ignoreFlg: boolean = false;
    ...
    _initHandWrite() {
        ...
        $('body').off('touchstart').on('touchstart', (e) => {
            ...
            selfHwp.isInputTouched && (<HTMLElement>document.activeElement).blur();
    ...
    // 手書き表示パネルを表示する。
    _showAssisArea(isInputToInput: boolean = false) {
        ...
        var type: string = selfHwp.currentElement.getAttribute(selfHwp.attrType);
        ...
        var mode: any = selfHwp.modeDef.keyboard;
    ...
    // 手書き表示パネルを非表示する。
    _closeAssisArea(isInputToInput: boolean = false) {
