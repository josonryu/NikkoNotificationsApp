...
function(AppComDevice, AppComClientDb, AppComHttp, AppComDate, LOG_LEVEL_SETTING, LOG_LEVEL_ORDER, appConst, APL_KBN, AppComStringUtil) {
     ...
     * @return void
     */
    var _writeLog = function(proper: string, shop: string, section: string, msgLevel: string,
        msg: string, action: string, ido: string, keido: string): void {

        // メッセージレベル未指定の場合は即時．処理を終了．
        if (!msgLevel) return;

        var logLvlSetting: number = LOG_LEVEL_ORDER[LOG_LEVEL_SETTING];
        var logLvlTarget : number = LOG_LEVEL_ORDER[msgLevel];

        // パラメータのメッセージレベルが指定出力レベルより低い場合は即時，処理を終了する．
        if (!logLvlSetting || !logLvlTarget || logLvlSetting > logLvlTarget) return;

        var time: string = AppComDate.getCurrentTimeMillis().replace(/[\D]/g, '');
        
        // アプリ区分
        var aplKbn: string = APL_KBN;
    ...
    return {
         ...
         * @return void
         */
        init: function(successCallback: any, errorCallback: any): void {

            // ログDB接続オブジェクトを初期化．
            if (!logDb) logDb = AppComClientDb.openTarget('aplLog.db', 'default');
         ...
         * @return void
         */
        writeLog: function(proper: string, shop: string, section : string, msgLevel: string,
            msg: string, action: string): void {
         ...
         * @return void
         */
        writeLogWithGps: function(proper: string, shop: string, section: string, msgLevel: string,
            msg: string, action: string, ido: string, keido: string): void {
         ...
         * @return void
         */
        uploadLog: function(successCallback: any, errorCallback: any): void {

            var time: string = AppComDate.getCurrentTimeMillis().replace(/[\D]/g, '');
            var uuid: string = AppComDevice.getUuid();
            var sql : string =
            `
                SELECT PROPER_CD,
                    UKETSUKE_MISE_CD,
            ...
            // DBからログを取得．
            AppComClientDb.execSQL(logDb, sql, [time], function (resultSet: any) {
                var count: number = resultSet.rows.length;

                // 取得結果なしの場合は即時，正常終了．
                if (count == 0) return successCallback( { RESULT_CODE: 1 }, '', '' );
                ...
                var current = {
                    PROPER_CD          : '', // 社員ID
                    UKETSUKE_MISE_CD   : '', // 受付者店部課コード
                    UKETSUKE_KAKARI_CD : '', // 受付者係コード
                    LOG_RECORD         : [], // ログレコード
                };
                ...
                for (var i: number = 0; i < count; i++) {
                    ...
                    if (current.PROPER_CD != row.PROPER_CD
                        ...
                        // 別ユーザのグルーピングを開始．
                        current = {
                            PROPER_CD          : row.PROPER_CD,          // 社員ID
                            UKETSUKE_MISE_CD   : row.UKETSUKE_MISE_CD,   // 受付者店部課コード
                            UKETSUKE_KAKARI_CD : row.UKETSUKE_KAKARI_CD, // 受付者係コード
                            LOG_RECORD         : [],                     // ログレコード
                        }
                    ...
                    current.LOG_RECORD.push({
                        LOG_TIME : row.LOG_TIME, // 操作日時
                        IDO      : row.IDO,      // 緯度
                        KEIDO    : row.KEIDO,    // 経度
                        APL_KBN  : row.APL_KBN,  // アプリ区分
                        LEVEL    : row.LEVEL,    // ログレベル区分
                        MESSAGE  : row.MESSAGE,  // メッセージ
                        ACTION   : action,       // アクションログ
                    });
                }

                // ReDOSサーバにログを送信．
                AppComHttp.post(appConst.SUBIF010.PATH, subif010Send,
                    function (subif010Recv: any, status: any) {
         ...
         * @return void
         */
        deleteLog: function(datetime: string, successCallback: any, errorCallback: any): void {

            // 削除対象日時が未指定の場合は即時，正常終了．
            if (!datetime) return successCallback();

            var time: string = datetime.replace(/[\D]/g, '');
...
.factory('AppComDevice', ['$filter', function($filter) {
    ...
    return {
        ...
        getNetworkStatus: function () {
            return (<any>navigator).connection.type;
        },
        ...
        getDeolocation: function (successCallback, errorCallback) {
            navigator.geolocation.getCurrentPosition(
                function (position) {
                    var result : any = {coords : {latitude : "", longitude : ""}};
...
.service('AppComClientDb', function() {
    ...
    this.openTarget = function (nm, locat){
        return (<any>window).sqlitePlugin.openDatabase({name: nm, location: locat});
    };
...
.factory('AppComClientSls', [function() {
    return {
         ...
         * @param {errorCallback} errorCallback - 失敗時のコールバック関数
         */
        setObject: function (time: string, type: string, value: Object, successCallback: any, errorCallback: any) {
         ...
         * @param {errorCallback} errorCallback - 失敗時のコールバック関数
         */
        getObject: function (key: string, successCallback: any, errorCallback: any) {
            sls.getItem(
                function (result) {
         ...
         * @param {errorCallback} errorCallback - 失敗時のコールバック関数
         */
        removeItem: function (key: string, successCallback: any, errorCallback: any) {
            sls.removeItem(
                successCallback, 
                errorCallback, 
                key);
         ...
         * @param {errorCallback} errorCallback - 失敗時のコールバック関数
         */
        getKeyLength: function (successCallback: any, errorCallback: any) {
            sls.length(
                successCallback, 
                errorCallback);
        },
         ...
         * @param {errorCallback} errorCallback - 失敗時のコールバック関数
         */
        getKey: function (index: number, successCallback: any, errorCallback: any) {
            sls.key(
                successCallback, 
                errorCallback,
                index);
...
.factory('AppComHttp', ['$http', 'CURRENT_CONFIG', 'AppComDate', 'AppComEncrypt', function ($http, CURRENT_CONFIG, AppComDate, AppComEncrypt) {
    ...
    return {
        ...
        post: function (apiUrl, params, successCallback, errorCallback, isMultiPart = false) {
            ...
            $http.post(CURRENT_CONFIG.API_URL + apiUrl, params, config).then(
                function (result) {
                    ...
                    if (result.headers('content-type').indexOf('application/json') === -1) {
                        try {
                            // ログ用DB開く、テーブル作成
                            var db = (<any>window).sqlitePlugin.openDatabase({ name: 'aplLog.db', location: 'default' });
                            db.executeSql('CREATE TABLE IF NOT EXISTS LOG (PROPER_CD TEXT NOT NULL, UKETSUKE_MISE_CD TEXT NOT NULL, UKETSUKE_KAKARI_CD TEXT NOT NULL, LOG_TIME TEXT NOT NULL, IDO TEXT, KEIDO TEXT, APL_KBN TEXT NOT NULL, LEVEL TEXT NOT NULL, MESSAGE TEXT NOT NULL, ACTION TEXT NOT NULL, PRIMARY KEY(PROPER_CD, LOG_TIME))', [], function () {
                                ...
                                db.executeSql('INSERT INTO LOG(PROPER_CD, UKETSUKE_MISE_CD, UKETSUKE_KAKARI_CD, LOG_TIME, IDO, KEIDO, APL_KBN, LEVEL, MESSAGE, ACTION) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                                    ['noUser', '000', '0000', logTime, null, null, '2', 'E', 'レスポンスのContent-Typeがapplication/jsonではない', '-'], function () {
    ...
    return {
        ...
        convertDate : function(strGengo, strYear, strMonth, strDay) {
            ...
            if (isNum(strGengo) && isNum(strYear) && isNum(strMonth) && isNum(strDay) && (minGengo <= strGengo && maxGengo >= strGengo)) {
                ...
                var startYmd : string = appConst.gengo["gengo" + strGengo].start;
                var endYmd : string = appConst.gengo["gengo" + strGengo].end;
                // 開始年月日の年を取得する
                var startYear : number = parseInt(startYmd.substring(0,4));
                // 計算した年
                var retYear : string = String(startYear + parseInt(strYear) - 1);
        ...
        nisaStartDate : function() {
            var nisastartdate : string = appConst.nisaFlagStartDate;
...
// 「APPCOM-0009：文字列共通処理」
.factory('AppComStringUtil', ['stringUtilConst', function (stringUtilConst: any) {
     ...
     * @param {any}    map  - 変換マップ
     */
    function convertChar(word: string, map: any): string {
        if (!word || !map || !Object.keys(map).length)
            return word;
    
        var result: string = '';
        var list: Array<string> = word.match(stringUtilConst.REGEXP);
        var length: number = list.length;
        var i: number = 0;

        while (i < length) {
            // 文字を取得
            var current: string = list[i];

            // 濁点付きの半角カナを取得
            var target: string = current + list[i + 1];
            if (target in map) {
                current = target;
                i = i + 1;
            }
    ...
    return {
         ...
         */
        hanToZen(word: string, isConvertKAN: boolean = false): string {
            var result: string = convertChar(word, stringUtilConst.HALF_STR);
            if (isConvertKAN) {
                // カナ文字を変換
         ...
         * @return {string}  result       - 変換後文字列
         */
        zenToHan(word: string, isConvertKAN: boolean = true): string {
            var result: string = convertChar(word, stringUtilConst.FULL_STR);
            if (isConvertKAN) {
                // カナ文字を変換
         ...
         * @return {string}  result       - 変換後文字列
         */
        smallToLarge(word: string): string {
