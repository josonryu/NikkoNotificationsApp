...
App.directive('inputData', ['$compile', '$parse', 'AppBizCom', 'AppLgcMultiCheck', 'AppCom', ($compile, $parse, AppBizCom, AppLgcMultiCheck, AppCom) => {
    ...
    return {
        restrict: 'A', // プロパティ
        priority: 1000, // MaxValue、ngRepeat以外一番前にInvoke
        require: ['inputData', 'ngModel'],
        link: function (scope, elem, attrs, ctrls) {

            // データを取得
            var nkData = $parse((<any>attrs).inputData)(scope);
            ...
            if (elem[0].tagName === 'INPUT') {
                ...
                if (nkData.handWrite != undefined) {
                    ...
                    var handWriteInfo = {
                        id: (<any>attrs).id,
                        pageID: (<any>attrs).pageId || findPageID(elem),
                        jpName: nkData.name,
                ...
                if (nkData.requireString) {
                    ...
                    ctrls[1].$render = function(){
                        var htmlEl = <HTMLInputElement>document.getElementById((<any>attrs).id);
                ...
                if (nkData.needRequireCheck){
                    var destoryListener = scope.$on((<any>attrs).id + 'AnyTimeChk', function (evnet, args) { 
        ...
        controller: function ($scope, $element, $attrs) {
            ...
            this.ele = <HTMLInputElement>document.getElementById($attrs.id);
             ...
             */
            this.anyTimeChk = function (prop: string, skipSimilarChk = false, self = this, isSimilarOnly = false) {
             ...
             * @param {string} prop - チェックを実行のプロパティ(valChangChk、onBlurChk)
             */
            this.registSimlarInfo = (chkResult, prop, self = this): void => {
