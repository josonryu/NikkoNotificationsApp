...
class NumKeyboard {
    ...
    // 数値キークリック
    _inputNumValue(e) {
        var val: string = myself.inputTarget.val();
        // 最大入力文字数確認
    ...
    _displayNumKeyboard(rect) {
        // 対象要素の位置取得
        var positionX: number = rect.left;
        var positionY: number = rect.top;
        // 対象要素の幅取得
        var targetWidth: number = rect.width;
        var targetHeight: number = rect.height;
        // 上右下部表示設定
        var keyboardPositionX: number = 0;
        var keyboardPositionY: number = 0;
        this.$triangle.removeClass();
        this.$triangle.css('left', '');

        // スクロールフラグ
        var canScroll = true;
        var keyboardAreaHeight: number = this.keyboardHeight + this.triangleHeight / 2;
        var scrollToY: number = positionY <= keyboardAreaHeight ? this.windowHeight - (positionY + keyboardAreaHeight + targetHeight) - 1 : 0;
        var scrollArea: JQuery = $('.scrollArea');
        ...
        if (myself.inputTarget.hasClass('numkeyboard-right') && (canRight || myself.inputTarget.hasClass('password-input'))) {
            ...
            // 左右位置調整用変数
            var correctionX: number = 0;
            var keyboardAddClassName: string = 'numKeyboard_reTriangle';
            var correctionY = 0;
            ...
            var diff: number = positionX + targetWidth / 2 + this.halfKeyboardWidth + 30 - this.documentWidth;
            if (diff > 0) {
