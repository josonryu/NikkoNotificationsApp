...
App.controller('selectIdentificationDocController', ['$scope', '$controller', 'AppBizCom', 'logicCom',
    function ($scope, $controller, AppBizCom, logicCom) {
        // 画面ID定義
        var MAIN_PAGE_ID = 'G1140-01'; // 本人確認書類のご説明・ご選択画面
        var PRE_PAGE_ID = 'G1110-01'; // 個人番号入力画面
        var NEXT_PAGE_ID = '';
        var CONFIRM_MODAL_ID = 'G1140-02'; // 撮影枚数確認画面
        ...
        // 本人確認書類マスタデータ取得
        var idTypes = AppBizCom.MstData.getCodeMstDataByKbn('HONIN_KAKNIN_SY');
        // 初期化処理
        var init = () => {
            scrollUnlock();
            // 注意喚起メッセージ表示フラグ初期化
            ...
            // 顔写真あり
            $scope.define1 = {};
            var withFace = idTypes.filter(idTypes => idTypes.STM2 === '1');
            var prefixD1 = 'firstType';
            $.each(withFace, (index, value) => {
            ...
            var selectedInfo = applyInfo.HONIN_KAKNIN_SY_JOHO;
            // 個人番号登録有無設定
            // 個人番号告知する場合
            ...
            if (selectedInfo && selectedInfo.HONIN_KAKNIN_SY_K_1) {
                // 書類選択状況の整合性チェック
                var consistencyCheck = false;
                var isWithFace = AppBizCom.MstData.getCodeMstDataByCd('HONIN_KAKNIN_SY', selectedInfo.HONIN_KAKNIN_SY_K_1).STM2 === '1';
                if (isWithFace) {
        ...
        // 共通領域保存処理
        var setNotifInfo = (idType1, idType2) => {
            // 申込データ(事務手続き)取得
            var applyInfo = AppBizCom.DataHolder.getNotifInfo();
            // 本人確認選択情報設定
        ...
        $scope.selectInfo = id => {
            ...
            var tmp1 = $scope.define1[id];
            var tmp2 = $scope.define2[id];
            // ----- 顔写真あり -----
            if (tmp1) {
                // 書類選択時
                if (tmp1.value) {
