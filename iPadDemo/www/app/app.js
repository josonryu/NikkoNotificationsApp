...
App.run(['$rootScope', 'AppBizDataHolder', 'timeOutCheck',
    function ($rootScope, AppBizDataHolder, timeOutCheck) {
        ...
        function setRouteInfo(current, previous) {
            if (previous) {
                var preUrl = previous.originalPath;
                var curUrl = current.originalPath;
                if (preUrl && curUrl) {
                    var prevPath = getPath(preUrl);
                    var curPath = getPath(curUrl);
                    var prevPathInHoler = AppBizDataHolder.getPrevRoutePath();
                    if (prevPathInHoler) {
