...
App
    ...
    function ($rootScope, $q, $location, appConst, appDefine, AppCom, AppBizCom, AppBizCodeMstData, LOG_LEVEL, serviceTimeCheck, timeOutCheck) {
         ...
         * @returns void
         */
        var writeActionLog = (fromId, toId, name, msgLevel, msgId, action, outputGps = false) => {
            var json = jsonParse(action);
            var msgParam = [fromId, toId, name];
         ...
         * @param {boolean} outputGps - ログ緯度経度出力実施フラグ(デフォルト：false[GPS無効])
         */
        var writeAplLog = (msgLevel, msg, param, outputGps = false) => {
            var json = jsonParse(param);
            writeBizLog(msgLevel, msg, json, outputGps);
        };
        /**
         * JSONを文字列に変換する．
         ...
         */
        var jsonParse = (action) => {
            // JSONを文字列に変換．
            var convUndef = function (k, v) { if (v === undefined) {
                return 'undefined';
            } return v; };
            // undefinedの場合，値を文字列を'undefined'に変換．
            var json = action === undefined ? '-' : JSON.stringify(action, convUndef);
            // オブジェクトがない場合，固定値'-'を設定．
            json = json == '{}' ? '-' : json;
            return json;
        };
         ...
         * @param {boolean} outputGps - ログ緯度経度出力実施フラグ(デフォルト：false[GPS無効])
         */
        var writeBizLog = (msgLevel, msg, action, outputGps = false) => {
            // ログイン者データの取得．
            ...
            // 位置情報の取得を試みる．
            AppCom.Device.getDeolocation(function (result) {
                // 成功した場合．
                var location = {
         ...
         * @returns void
         */
        var getSLSKeys = (type, onSuccess, onError) => {
            var keys = [];
            var startIndex = 0;
         ...
         */
        var fileSystemErrorHandler = (error) => {
         ...
         */
        var conmbinedImage = (fillImage, orgImage) => {
            var BASE64_HEAD = 'data:image/jpeg;base64,';
            var regExpDelete = new RegExp(BASE64_HEAD, 'g');
            var setHeight = orgImage.height;
            var setWidth = orgImage.width;
            // 画像を結合
        ...
        return {
             ...
             * @returns void
             */
            btnTapLog: (fromId, toId, name, action, outputGps = false) => {
             ...
             * @returns void
             */
            btnTapErrLog: (fromId, toId, name, action, outputGps = false) => {
             ...
             * @returns void
             */
            btnTapNextPageErrLog: (fromId, toId, name, action, outputGps = false) => {
             ...
             * @returns void
             */
            callbackLog: (fromId, toId, name, param, outputGps = false) => {
             ...
             * @returns void
             */
            callbackErrLog: (fromId, toId, name, msgId, param, outputGps = false) => {
                var json = jsonParse(param);
                var msgParam = [fromId, toId, name, msgId];
             ...
             * @returns void
             */
            debugLog: (msg, param, outputGps = false) => {
             ...
             * @returns void
             */
            infoLog: (msg, param, outputGps = false) => {
             ...
             * @returns void
             */
            warnLog: (msg, param, outputGps = false) => {
             ...
             * @returns void
             */
            errorLog: (msg, param, outputGps = false) => {
             ...
             * @returns void
             */
            getSLSLength: (type, onSuccess, onError) => {
                // ①　成功した場合
             ...
             * @returns void
             */
            getSLSDatas: (keys, onSuccess, onError) => {
                var promises = [];
                var len = keys.length;
             ...
             */
            deleteSlsData: (keys, onSuccess, onError) => {
                var index = 0;
            ...
            existsFile: (path, callback) => {
                ...
                // ファイル操作処理
                window.resolveLocalFileSystemURL(path, fileExistsFunc, fileNotExistsFunc);
                return;
            },
             ...
             */
            createDirectory: (path, dirName, successCallback, errorCallback) => {
                ...
                // ファイル操作処理
                window.resolveLocalFileSystemURL(path, successFunc, errorFunc);
                return;
            },
            fileDelete: (filePath, successCallback, errorCallback) => {
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            fileCopy: (srcFilePath, dstDirPath, dstFileName, successCallback, errorCallback) => {
                ...
                var srcFsSuccessFunc = function (fsSrc) {
                    ...
                    window.resolveLocalFileSystemURL(dstDirPath, dstFsSuccessFunc, dstFsErrorFunc);
                };
                // コピー元ファイルシステム処理失敗時
                ...
                var dstFsErrorFunc = function (e) {
                    ...
                    if (e.code == 12) {
                        // DBファイルを削除してからコピーを実施
                        fileSystemTarget.getFile(dstFileName, { create: false }, function (fileEntry) {
                ...
                window.resolveLocalFileSystemURL(srcFilePath, srcFsSuccessFunc, srcFsErrorFunc);
            },
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            directoryDelete: (deleteDirPath, successCallback, errorCallback) => {
                var fileSystemDelete;
                ...
                // 削除ディレクトリ ファイルシステム取得
                window.resolveLocalFileSystemURL(deleteDirPath, srcFsSuccessFunc, srcFsErrorFunc);
            },
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            directoryCopy: (srcDirPath, dstDirPath, dstDirName, successCallback, errorCallback) => {
                var fileSystemSource;
                var fileSystemTarget;
                // コピー元ファイルシステム処理成功時
                var srcFsSuccessFunc = function (fsSrc) {
                ...
                // コピー元 ファイルシステム取得
                window.resolveLocalFileSystemURL(srcDirPath, srcFsSuccessFunc, srcFsErrorFunc);
            },
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            saveFileTxtData: (dirPath, fileName, txtData, successCallback, errorCallback) => {
                var strSrc = [txtData];
                // TXT形式データのBlob型変換
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            readFileTxtData: (filePath, successCallback, errorCallback) => {
                window.resolveLocalFileSystemURL(filePath, function (fileEntry) {
                    // ファイルシステムオプション
                    var options = {
                        exclusive: false,
             ...
             * @param  {any} errorCallback - 処理成功時コールバック
             */
            saveFileBase64DataBat: (dirPath, filesInfo, successCallback, errorCallback) => {
                // ファイル数
                ...
                window.resolveLocalFileSystemURL(dirPath, function (dir) {
                    ...
                    var createFunc = function () {
                        var oneInfo = filesInfo[createFileCount];
                        ...
                        dir.getFile(oneInfo.fileName, options, function (file) {
                            file.createWriter(function (fileWriter) {
                                fileWriter.write(dataBlob);
                                createFileCount++;
                                // 画像が全てそろった場合
             ...
             */
            setConmbinedImage: (imgList, i, callback) => {
                ...
                new Promise((resolve, reject) => {
                    ...
                    fillImage.onload = function () {
                        orgImage.onload = function () {
                            // 共通領域に設定
                            var comDataImageData = AppBizCom.DataHolder.getImageData();
            ...
            autoMask: (idDocType, surface) => {
                ...
                canvas.height = 634;
                canvas.hidden = true;
                var parent = document.getElementById('main-div-area');
                parent.appendChild(canvas);
                var fillImageClass = new AppBizImageFillMask('canvas');
                // 自動マスク画像作成
                fillImageClass.autoMask(idDocType, surface);
                // BASE64で取得
                var base64Data = fillImageClass.getImageData();
                // BASE64のヘッダーは不要
                var BASE64_HEAD_PNG = 'data:image/png;base64,';
                var regExpDelete = new RegExp(BASE64_HEAD_PNG, 'g');
                // 塗りつぶし済み画像をセット
                var imageBase64 = typeof (base64Data) != 'undefined' ? base64Data.replace(regExpDelete, '') : appConst.NO_INPUT_VALUE;
                // 共通領域定義 申込みデータ 項目名定数取得
                var d = appDefine.imageData;
                // 共通領域定義 申込みデータ取得
                var comDataImageData = AppBizCom.DataHolder.getImageData();
                // 共通領域へ保存
                if (surface === 1) {
             ...
             * @returns void
             * */
            setOcrResult2DataHolder: (ocrResult, onSuccess, onError) => {
                var c = appConst.CARD_TYPE;
                var d = appDefine.ocrResultData;
                var ocrValues = {};
                ...
                var sep = appConst.OCR_SEPALATE_CHAR; //'　';
                // 氏名の取得
                var split_wk = [undefined, undefined];
                // 氏名:苗字名前を分割して設定
                ...
                // 氏名カナの取得
                var unknownCheck = appConst.OCR_UNKNOWN_CHAR; //'？';
                ...
                var birthday = ocrResult.birthday.text;
                ...
                if (AppBizCom.InputCheck.isEmpty(split_wk) || split_wk.length != 3) {
                    ...
                    // 元号はコードマスタから取得して変換する
                    var mst = AppBizCodeMstData.getCodeMstDataByKbn('GNGO_K');
                    var len = mst.length;
                    for (var i = 0; i < len; i++) {
                        // 和暦
                ...
                var address = ocrResult.address.text;
                if (AppBizCom.InputCheck.isEmpty(address)) {
                    ...
                    AppBizCom.AddSearch.searchFromAdd(address, function (result) {
             ...
             * @returns void
             * */
            setOcrResult2DataHolderJimu: (ocrResult, callback) => {
                var d = appDefine.ocrResultData;
                var ocrValues = {};
                // 撮影モードの取得
             ...
             * @returns void
             */
            change2CameraPreview: (divId, modalId, camId) => {
                // StatusBar.hide();
             ...
             * @returns void
             */
            change2DefaultView: (divId, modalId, camId) => {
                // StatusBar.show();
             ...
             */
            locationPath: (path, onSuccess, onError, onConnectionError, isShowIndicator = true) => {
                // // アプリ起動時のパスを判別
                ...
                // }
                // // サービス時間チェック
                // serviceTimeCheck.checkServiceTime(() => {
                    // ログ出力
    ...
    .factory('chkSimilar', ['AppBizCom', 'chkSimilarConst', function (AppBizCom, chkSimilarConst) {
         ...
         */
        var createDefineList = (chkType, checkTypeList) => {
            if (checkTypeList == undefined)
                checkTypeList = {};
            if (checkTypeList[chkType])
                return;
            // 類似文字チェックタイプ定義でループ
            ...
            for (var j = 0; j < SIMILAR_CHAR_DEFINE.length; j++) {
                ...
                var chkGroup = SIMILAR_CHAR_DEFINE[j].filter(item => item.type & chkType);
         ...
         * @return {Array} チェック結果
         */
        var chkSimilarChar = function (chkStr, checktargetDefine) {
            var chkList = undefined;
            // （絵文字対応）対象文字列を配列に変更する
        ...
        return {
             ...
             */
            charCheckResults: function (id, inputCharList, chkType) {
                // inputCharListチェック
    ...
    function (AppBizInputCheck, AppBizEmojiUtil, AppBizMsg, chkSimilar) {
         ...
         */
        var linkCheck = (chkObj) => {
            var errFlg = false;
            var tmpId = chkObj.id + 'Similar';
         ...
         */
        var multiInputCheck = (chkObjs, inputObjs) => {
            ...
            var errFlg = false; // チェック結果: エラーフラグ
            var msgParam = {}; // チェック結果: 詳細
            for (var target in chkObjs) {
                var chkObj = chkObjs[target];
                ...
                msgParam[target] = {};
                // 該当項目の単項目チェック
                var result = inputCheck(chkObj, 'allChk');
                // 該当項目の類似文字確認済みチェック
                var linkChkErr = false;
        ...
        var inputCheck = (chkObj, type) => {
            ...
            var msgId = []; // エラーメッセージコード
            var msgParam = {}; // 入力チェック結果
            var similarCharParam = {}; // 類似文字チェック結果
            // チェックタイプによって、チェックリストを取得
            var chkList = getCheckList(chkObj, type); // 画面項目定義.チェックリスト
            var chkId = chkObj.id; // 画面項目定義.画面項目ID
            var chkName = chkObj.name; // 画面項目定義.画面項目名
            var chkErrAreaId = chkObj.errAreaId; // 画面項目定義.エラーメッセージ表示用画面項目ID
            var chkTypeSelect = chkObj.typeSelect; // 画面項目定義.必須チェックエラータイプ
            var chkVal = chkObj.val; // 画面項目定義.エラーメッセージ表示用画面項目ID（文字列化）
            var chkValArray = AppBizEmojiUtil.toArray(String(chkVal));
            // チェック優先順位ループ
            for (var i = 0; i < chkList.length; i++) {
                var chkItem = chkList[i];
                var subList = (!Array.isArray(chkItem)) ? [chkItem] : chkItem;
                // 各単項目チェックループ
                for (var j = 0; j < subList.length; j++) {
                    var subItem = subList[j];
                    // メッソドチェックだったら、実行して結果を取得
                    var isFunc = chkFunction(subItem);
                    if (isFunc) {
                        var funcRes = subItem();
                    ...
                    switch (subItem) {
                        case 'isEmpty':
                            ...
                            if (AppBizInputCheck.isEmpty(chkVal)) {
                                var errId = chkTypeSelect ? 'KKAP-CM000-02E' : 'KKAP-CM000-01E';
                                msgId.push(errId);
                                var errMsg = AppBizMsg.getMsg(errId, [chkName]);
                        ...
                        case 'isFullHurigana':
                            ...
                            if (!AppBizInputCheck.isEmpty(chkVal) && !AppBizInputCheck.isFullHurigana(chkVal)) {
                                var errId = 'KKAP-CM000-03E';
                                msgId.push(errId);
                                var errMsg = AppBizMsg.getMsg(errId, [chkName, '全角カナ']);
                                showErr(chkId, chkErrAreaId, errId, errMsg);
                            }
                            break;
                        ...
                        case 'isFullString':
                            ...
                            if (!AppBizInputCheck.isEmpty(chkVal) && !AppBizInputCheck.isFullString(chkValArray)) {
                                var errId = 'KKAP-CM000-03E';
                                msgId.push(errId);
                                var errMsg = AppBizMsg.getMsg(errId, [chkName, '全角']);
                                showErr(chkId, chkErrAreaId, errId, errMsg);
                            }
                            break;
                        case 'isNum':
                            ...
                            if (!AppBizInputCheck.isEmpty(chkVal) && !AppBizInputCheck.isNum(String(chkVal))) {
                                var errId = 'KKAP-CM000-03E';
                                msgId.push(errId);
                                var errMsg = AppBizMsg.getMsg(errId, [chkName, '数字']);
                                showErr(chkId, chkErrAreaId, errId, errMsg);
                            }
                            break;
                        case 'chkMaxLength':
                            ...
                            if (!AppBizInputCheck.isEmpty(chkVal) && AppBizInputCheck.chkMaxLength(String(chkVal), chkObj.length) == 1) {
                                var errId = 'KKAP-CM000-09E';
                                msgId.push(errId);
                                var errMsg = AppBizMsg.getMsg(errId, [chkName, chkObj.length]);
                                showErr(chkId, chkErrAreaId, errId, errMsg);
                            }
                            break;
                        case 'chkSameLength':
         ...
         */
        var showErr = (chkId, chkErrAreaId, errId, errMsg, kbn = true) => {
            var errAreaId = chkErrAreaId ? chkErrAreaId : chkId;
        ...
        return {
             ...
             * @return {Array<any>} - チェック結果 [ エラーフラグ, 入力チェック結果 ]
             */
            multiInputCheck: (chkObjs, inputObjs) => {
     ...
     */
    var objAssign = (applyDefine, targetObj) => {
        ...
        for (var prop in targetObj) {
            if (targetObj.hasOwnProperty(prop) && applyDefine.hasOwnProperty(prop)) {
                var value = targetObj[prop];
                var define = applyDefine[prop];
                if (typeof (value) === 'object' && value != null && Object.keys(value).length != 0) {
                    if (Array.isArray(value) && value.length != 0 && Array.isArray(define)) {
                        var tmpLen = (define.length > value.length) ? value.length : define.length;
                        result[prop] = [];
    ...
    return {
         ...
         * @return {any} - 割り当てる結果
         */
        objAssign: (applyDefine, targetObj) => {
    ...
    function ($controller, AppBizCom, appConst, APL_NAME, LOG_LEVEL, AppComLog) {
        return {
             ...
             */
            checkServiceTime(onSuccess, onError, onConnectionError, isShowIndicator = true) {
                // ログイン者データの取得．
                var loginInfo = AppBizCom.DataHolder.getLoginInfo();
                var proper = loginInfo.PROPER_C;
                var shop = loginInfo.UKETSUKE_MISE_C;
                ...
                AppBizCom.CheckServTime.checkServiceTime(function () {
                    ...
                    if (typeof error == "object" && 'RESULT_CODE' in error) {
                        if (error.RESULT_CODE == appConst.SUBIF001.RESULT_CODE.OFFLINE) {
                            ...
                            var SRVC_KS_ZKK = serviceTime.SRVC_KS_ZKK;
                            var serviceSta = Number(SRVC_KS_ZKK.substr(0, 2)) + ':' + SRVC_KS_ZKK.substr(3, 2);
                            var SRVC_SYRY_ZKK = serviceTime.SRVC_SYRY_ZKK;
                            var serviceEnd = Number(SRVC_SYRY_ZKK.substr(0, 2)) + ':' + SRVC_SYRY_ZKK.substr(3, 2);
                            // サービス時間外メッセージの定義
