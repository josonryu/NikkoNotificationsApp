...
class HandWritePanelNew {
    constructor(stringUtil) {
        ...
        this.isHandMode = false; // 手書きモード区分
        this.isCloseKeyboard = false; // OSキーボード閉じる状態フラグ
        this.tmpCloseElement = undefined; // OSキーボード閉じる中の対象要素
        this.isOpenHandMode = false; // モード切り替え時のフラグ
        this.isComposing = false; // 未確定文字フラグ
        selfHwp = this;
    ...
    _initHandWrite() {
        ...
        $('body').off('touchend').on('touchend', (e) => {
            ...
            var isInputTouched = e.target.getAttribute(selfHwp.attrType) != null;
            var tmpTar = $(e.target).parents('#modal-assistant');
            // ボタン要素について、フォーカス取得可能にする
            var isFocusChangeElem = e.target.tagName === 'BUTTON';
