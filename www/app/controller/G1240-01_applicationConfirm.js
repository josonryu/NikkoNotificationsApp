...
App.controller('applicationConfirmController', ['$scope', '$controller', '$anchorScroll', '$timeout', 'AppBizDataHolder', 'AppBizMsg', 'logicCom', 'AppComDevice', 'AppBizCodeMstData',
    function ($scope, $controller, $anchorScroll, $timeout, AppBizDataHolder, AppBizMsg, logicCom, AppComDevice, AppBizCodeMstData) {
        ...
        var errorCheck = function () {
            ...
            var errSyuruiTitle = ('1' === getString(inputFlgControl.KYAKNM_F)
            ...
            // 画面エラー制御
            $scope.pageErrCtl = {
                wholeHasError: wholeHasError,
                errNikoCardPin: errNikoCardPin,
                errMynumber: errMynumber,
                errSyuruiTitle: errSyuruiTitle,
                errMynumberCard: errMynumberCard,
            };
            //　画面エラーメッセージ
            $scope.pageErrMsg = {
                wholeErrMsg: wholeErrMsg,
                wholeErrMsgId: wholeErrMsgId,
                errNikoCardPinMsg: errNikoCardPinMsg,
                errMynumberMsg: errMynumberMsg,
                errSyuruiTitleMsg: errSyuruiTitleMsg,
                errMynumberCardMsg: errMynumberCardMsg,
            };
        };
        // 画面初期化
        var pageInit = function () {
            ...
            var clearCheck1 = !('1' === getString(inputFlgControl.KYAKNM_F)
                || '1' === getString(inputFlgControl.KYAK_ADDR_F)
                || ('1' === getString(inputFlgControl.TKTEI_KOZA_F) && '1' === getString(jimuJoho.TKTEI_KOZA_MSKM))
                || '1' === getString(inputFlgControl.NISA_KOZA_F)
                || '1' === getString(inputFlgControl.MYNO_KOKUCHI_F))
            ...
            var clearCheck2 = ('1' === getString(jimuJoho.MYNO_KOKUCHI)
            ...
            */
            $scope.clearMynumberJoho =
                (((('0' === getString(customer.MYNO_KKC)) && ('0' === getString(inputFlgControl.MYNO_KOKUCHI_F)))
                    || (('1' === getString(customer.MYNO_KKC)) && (('1' !== getString(jimuJoho.MNSYSEIRY_JISN_FLAG)))))
                    && ('' !== getString(personInfo.MYNO))) || (('0' === getString(inputFlgControl.MYNO_KOKUCHI_F))
                    && ('1' !== (getString(jimuJoho.MNSYSEIRY_JISN_FLAG)))
                    && ('' === getString(personInfo.MYNO))
        ...
        var pageShowInit = function () {
            ...
            // 顧客契約情報のホスト帳票閲覧店権限管理チェック結果が"1：ホスト帳票閲覧店権限あり"の場合、「外国証券の円貨利金分配金振込先」の変更するボタンを表示する。
            $scope.showBtnGaikSyknYen = '1' == getString(customer.CHOHYO_ETRN_CHK);
            // 顧客契約情報のホスト帳票閲覧店権限管理チェック結果が"1：ホスト帳票閲覧店権限あり"の場合、「分配金受取方法指定（累投）」の変更するボタンを表示する。
        ...
        var pageEventInit = function () {
            ...
            $('[data-target]').click(function () {
                if ($scope.pageErrCtl.wholeHasError || inputSignBtnClicked) {
                    return;
                }
                $timeout(function () {
                    if (inkCanvas) {
            ...
            $scope.confirm = function () {
                ...
                editedPath = true;
                if ($scope.pageShow.KYAKNM_F === '1' || $scope.pageShow.KYAK_ADDR_F === '1' || $scope.pageShow.TELNO_F === '1') {
                    var warningModal = $('#G1080-24');
