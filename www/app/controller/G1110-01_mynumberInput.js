...
App.controller('mynumberInputController', ['$scope', '$controller', 'AppBizCom', 'logicCom', 'AppLgcMultiCheck', 'appConst', 'AppBizCamera',
    function ($scope, $controller, AppBizCom, logicCom, AppLgcMultiCheck, appConst, AppBizCamera) {
        // エラーモーダル用共通コントローラ
        $controller('errorInfoModalCtrl', { $scope: $scope });
        // 画面ID定義
        var MAIN_PAGE_ID = 'G1110-01'; // 個人番号入力画面
        var DISPLAY_MODAL_ID = 'G1120-01'; // 撮影書面表示（番号確認書類）画面
        var SHOOTING_PAGE_ID = 'G1130-01'; // カメラ撮影（番号確認書類）画面
        var PRE_PAGE_ID = 'G1100-01'; // 番号確認書類のご説明・ご選択画面画面
        var NEXT_PAGE_ID = '';
        ...
        var init = () => {
            ...
            function (error) {
                ...
                // カメラプラグイン終了
                AppBizCamera.deinitCamOcr(function () { }, function () { });
            });
            // 選択済み番号確認書類情報取得
            var d = AppBizCom.DataHolder.getNotifInfo();
        ...
        var shootingMynumOCR = () => {
            ...
            camInfo => {
                ...
                $scope.mynumberOcr = "000000000000";
                // OCR結果表示処理
                var headsUpID = '';
        ...
        var showError = (inputData, msgText) => {
        ...
        $scope.btnNextClick = () => {
            ...
            // ログ出力用「次へ」ボタン表示名
            var btnName = $scope.editMode ? '確認画面へ' : '次へ';
        ...
        $scope.initModal = () => {
            ...
            if (surface !== '') {
                // メッセージ取得
                var msgParam = [$scope.myNumDocInfos.MSY + 'の<span class="strong">' + surface + '</span>'];
                var msgText = AppBizCom.Msg.getMsg('KKAP-SF009-02I', msgParam);
                // メッセージ表示
                $('#G1120Title').html(msgText);
            }
            // イベント登録
            $('#' + DISPLAY_MODAL_ID).on('show.bs.modal', () => {
                $('body').addClass('is-modal-open');
            });
            $('#' + DISPLAY_MODAL_ID).on('hidden.bs.modal', () => {
                $('body').removeClass('is-modal-open');
                stopBtnEventFLG = false;
            });
            // イベント廃棄
