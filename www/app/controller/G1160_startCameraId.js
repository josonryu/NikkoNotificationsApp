...
App.controller('startCameraId', ['$scope', '$location', '$controller', '$sce', 'appConst', 'logicCom', 'AppBizCom', 'AppBizCamera',
    function ($scope, $location, $controller, $sce, appConst, logicCom, AppBizCom, AppBizCamera) {
        // 共通部分制御を継承
        $controller('errorInfoModalCtrl', { $scope: $scope });
        // 画面ID定義
        var DISPLAY_MODAL_ID_01 = 'G1160-01'; // 撮影書面表示画面
        var DISPLAY_MODAL_ID_02 = 'G1160-02'; // 撮影書面表示（スキップ時）画面
        ...
        // ボタン連打防止フラグ
        var inheritStopBtnEventFLG = false;
        ...
        $scope.initG1160 = () => {
            ...
            $('#G1160-01').on('hidden.bs.modal', () => {
                $('body').removeClass('is-modal-open');
            });
            $('#G1160-02').off('show.bs.modal');
            $('#G1160-02').on('show.bs.modal', () => {
                $('#G1160-02').off('shown.bs.modal');
                if ($('body').hasClass('is-modal-open')) {
                    $('#G1160-02').on('shown.bs.modal', () => {
                        $('body').removeClass('is-modal-open');
                        $('body').addClass('is-modal-open');
                    });
                }
        ...
        // メッセージ定義
        var TITLE_MESSAGE_ID = 'KKAP-SF011-01I';
        var BACK_SKIP_MESSAGE_ID = 'KKAP-SF011-02I';
        var SKIP_MESSAGE_ID = 'KKAP-SF011-03I';
        var BOOK_FRONT_MESSAGE_ID = 'KKAP-SF011-04I';
        var BOOK_BACK_MESSAGE_ID = 'KKAP-SF011-05I';
        // 遷移先定義
        var URL_FRONT_RESULT = 'cameraIdFrontResult';
        var URL_BACK_RESULT = 'cameraIdBackResult';
        var URL_THIRD_RESULT = 'cameraIdThirdResult';
        // 画面IDなどの設定用変数
        var startCameraPageId = '';
        var cameraShootingId = 'G1170-01';
        var cameraResultId = '';
        // 「G1160-01:撮影書面表示画面」/「G1160-02:撮影書面表示(スキップ時)画面」への遷移元画面id
        var prevPageId = '';
        // 撮影対象書面
        var targetIdentity = 0; // 1: １種類目 //2: ２種類目
         ...
         * @returns void
         */
        $scope.initIdentityDocInfo = (identityNum, from, targetPageId) => {
            ...
            // 撮影面
            var surfaceG1160 = appConst.CAM_DISPLAY_MODAL.FRONT_NAME;
            ...
            if ($scope.popupMode === 'FRONT') {
                ...
                if (personDocInfo.STM3 && personDocInfo.STM3.length < 3 && personDocInfo.CD !== '03' && personDocInfo.CD !== '13') {
                    ...
                    // 冊子型書面用メッセージ設定
                    if ($scope.isBookFlg) {
                        var bookMessage = AppBizCom.Msg.getMsg(BOOK_FRONT_MESSAGE_ID, []);
                ...
                if (personDocInfo.STM3 && personDocInfo.STM3.length < 3 && personDocInfo.CD !== '03' && personDocInfo.CD !== '13') {
                    ...
                    // スキップ案内設定
                    var backSkipMessage = AppBizCom.Msg.getMsg(BACK_SKIP_MESSAGE_ID, [personDocInfo.MSY]);
                    $scope.skipMessage = $sce.trustAsHtml(backSkipMessage);
                    // 冊子型書面用メッセージ設定
                    if ($scope.isBookFlg) {
                        var bookMessage = AppBizCom.Msg.getMsg(BOOK_BACK_MESSAGE_ID, []);
                    ...
                    // スキップ案内設定
                    var skipMessage = AppBizCom.Msg.getMsg(SKIP_MESSAGE_ID, [surfaceG1160]);
            ...
            // タイトル設定
            var titleParamG1160 = personDocInfo.MSY + 'の<span class="strong">' + surfaceG1160 + '</span>';
            $scope.titleG1160 = $sce.trustAsHtml(AppBizCom.Msg.getMsg(TITLE_MESSAGE_ID, [titleParamG1160]));
            // サンプル画像設定
            var imageType = AppBizCom.MstData.getCodeMstDataByCd('HONNIN_KAKNN_GAZO', kind);
        ...
        var takePicModeOcr = () => {
            ...
            camInfo => {
                ...
                logicCom.setOcrResult2DataHolderJimu(camInfo, () => {
                    ...
                    //     logicCom.change2DefaultView(prevPageId, startCameraPageId, cameraShootingId);
                    //     // カメラプラグイン終了
                    //     AppBizCamera.deinitCamOcr((): void => {}, (): void => {});
                });
                // 01-2022-06-224 本人確認書類撮影画面のガード対応 終了 20221031
            }, 
            // エラー発生時
