...
App.controller('bankSearchController', ['$scope', '$timeout', 'AppBizCom', 'logicCom', '$controller', 'AppBizMsg', 'AppLgcMultiCheck',
    function ($scope, $timeout, AppBizCom, logicCom, $controller, AppBizMsg, AppLgcMultiCheck) {
        ...
        $scope.showSearchFacilityResult = function () {
            ...
            var keyboardShowing = (undefined == window.cordova) ? false : (cordova.plugins.Keyboard.isVisible);
            // 入力項目「金融機関名」エラークリア
            AppBizCom.Msg.clearError('txtSearchWordFacility');
            ...
            if (!chkResult[0]) {
                ...
                AppBizCom.BankSearch.searchFromBkNm(inputVal, function (result) {
        ...
        $scope.showSearchBranchResult = function () {
            ...
            if (!chkResult[0]) {
                ...
                AppBizCom.BankSearch.searchFromBkCd(txtOrgCode, txtOrgName, inputVal, function (result) {
                    // アプリログ出力
                    var recordLth = result.rows.length; // レコード件数
        ...
        var bankSearchInputData = {
            TXT_ORG_WORD: {
                applyName: 'TXT_ORG_WORD',
                id: 'txtSearchWordFacility',
                name: '金融機関名',
                typeSelect: false,
                requireString: 'full',
                handWrite: 'text_all',
                length: 15,
                valChangChk: [['isFullString', 'hasForbidChar', 'chkMaxLength']],
                allChk: [['isEmpty'], ['isFullString', 'hasForbidChar', 'chkMaxLength']],
            },
            TXT_BRANCH_WORD: {
                applyName: 'TXT_BRANCH_WORD',
                id: 'txtSearchWordBranch',
                name: '支店名',
                typeSelect: false,
                requireString: 'full',
                handWrite: 'text_all',
