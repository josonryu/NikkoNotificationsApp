...
App.controller('selectSignDocController', ['$scope', '$controller', 'AppBizCom', 'appConst', 'logicCom', 'AppLgcMultiCheck', 'AppBizMsg', 'AppCom', 'APL_NAME',
    function ($scope, $controller, AppBizCom, appConst, logicCom, AppLgcMultiCheck, AppBizMsg, AppCom, APL_NAME) {
        ...
        /** 電子帳票対象書面選択画面. */
        var THIS_PAGE_ID = 'G1400-01';
        /** 電子帳票入力画面. */
        var NEXT_PAGE_ID = 'G1410-01';
        var NEXT_PAGE_PATH = 'signDoc';
        /** 申込中止画面（電子帳票）. */
        var STOP_MODAL_ID = 'G1400-03';
        /** ホームメニュー画面. */
        var HOME_PAGE_ID = 'G1040-01';
        var HOME_PAGE_PATH = 'homeMenu';
        /** 電子帳票対象書面選択のエラー項目表示画面. */
        var ERR_MODAL_ID = 'G1400-02';
        /** ボタン. */
        var NEXT_BTN_NAME = '次へ';
        var STOP_BTN_NAME = '中止する';
        var YES_BTN_NAME = 'はい';
        var NO_BTN_NAME = 'いいえ';
        var CLOSE_BTN_NAME = '閉じる';
        /** サービス時間情報. **/
        var serviceTime = AppBizCom.DataHolder.getServiceTime();
        var SRVC_KS_ZKK = Number(serviceTime.SRVC_KS_ZKK.substr(0, 2)) + ':' + serviceTime.SRVC_KS_ZKK.substr(3, 2);
        var SRVC_SYRY_ZKK = Number(serviceTime.SRVC_SYRY_ZKK.substr(0, 2)) + ':' + serviceTime.SRVC_SYRY_ZKK.substr(3, 2);
        /** システムエラー. */
        var ERR_SYSTEM = {
        ...
        /** エラーメッセージ. */
        var MSG_NONE = '電子帳票フォーマット取得結果なし';
        var MSG_OFFLINE = '電子帳票フォーマット取得結果サービス時間外エラー';
        var MSG_OTHER_ERR = '電子帳票フォーマット取得結果その他エラー';
        var MSG_NETWORK_ERR = '電子帳票フォーマット取得通信エラー';
        /**
         * HTML生成前の初期化処理
         *
         * @return void
         ...
         * @return void
         */
        $scope.changeMailInputMode = function () {
            ...
            if (!checkExistError('mailAddress1', 'mailAddress2', 'domainType', 'mailAddress1-mailAddress2-domainType')) {
                ...
                if ($scope.MODEL.MAIL_DOMAIN_FLAG == '1') {
                    if ($scope.MODEL.MAIL_ACCOUNT && $scope.MODEL.MAIL_DOMAIN) {
                        var maxErr = checkGroupMaxLength('mailAddress1-mailAddress2-domainType', 49, true, 'KKAP-SF014-10E', [], true, $scope.MODEL.MAIL_ACCOUNT, $scope.MODEL.MAIL_DOMAIN);
                        var minErr = checkGroupMinLength('mailAddress1-mailAddress2-domainType', 4, true, 'KKAP-CM000-15E', ['E-mailアドレス', 5], true, $scope.MODEL.MAIL_ACCOUNT, $scope.MODEL.MAIL_DOMAIN);
                    }
                    ...
                    if ($scope.MODEL.MAIL_ACCOUNT && $scope.MODEL.MAIL_DOMAIN_C) {
                        var codeMstData = AppBizCom.MstData.getCodeMstDataByCd('E_MAIL_DOMAIN', $scope.MODEL.MAIL_DOMAIN_C);
                        var domainName = codeMstData ? codeMstData.MSY : '';
                        var maxErr = checkGroupMaxLength('mailAddress1-mailAddress2-domainType', 49, true, 'KKAP-SF014-10E', [], true, $scope.MODEL.MAIL_ACCOUNT, domainName);
                        var minErr = checkGroupMinLength('mailAddress1-mailAddress2-domainType', 4, true, 'KKAP-CM000-15E', ['E-mailアドレス', 5], true, $scope.MODEL.MAIL_ACCOUNT, domainName);
                    }
                    if (maxErr == undefined && minErr != undefined) {
                        return minErr;
                    }
        ...
        var hasForbidMailLocalChar = function () {
            var result;
        ...
        // E-mailアドレス（ドメイン部）桁数
        var chkMinDomainLength = function () {
            // 桁数チェック
            var result;
        ...
        var hasForbidMailDomainChar = function () {
            var result;
            var mail2Value = $scope.MODEL.MAIL_DOMAIN;
        ...
        // E-mailアドレスクリアエラー
        var clearMailGroupError = function () {
         ...
         * @return void
         */
        var errorCallback = function (subIf006Recv, status) {
            // REDOSサーバ応答結果
            var receive = {
        ...
        $scope.nextBtnClick = function () {
            ...
            if (result[0] === true) {
                ...
                // ログイン者データ取得
                var eFormInfo = AppBizCom.DataHolder.getEFormInfo();
                var eigyoinJoho = eFormInfo.EIGYOIN_JOHO;
                // 電子帳票フォーマット取得要求インターフェース作成
                var subIf006Send = {
                    PROPER_C: eigyoinJoho.PROPER_C,
                    UKETSUKE_MISE_C: eigyoinJoho.UKETSUKE_MISE_C,
                    UKETSUKE_KAKARI_C: eigyoinJoho.UKETSUKE_KAKARI_C,
                    CHOHYO_ID: $scope.MODEL.CHOHYO_ID,
                };
                // REDOSサーバに帳票画像を要求
         ...
         * @return void
         */
        var setEFormInfo = function (subIf006Recv) {
            // 申込データに値を設定する
            var eFormInfo = AppBizCom.DataHolder.getEFormInfo();
            eFormInfo.CHOHYO = {
                CHOHYO_ID: $scope.MODEL.CHOHYO_ID,
                MAIL_ACCOUNT: $scope.MODEL.MAIL_ACCOUNT,
                MAIL_DOMAIN_FLAG: $scope.MODEL.MAIL_DOMAIN_FLAG,
                MAIL_DOMAIN: $scope.MODEL.MAIL_DOMAIN,
                MAIL_DOMAIN_C: $scope.MODEL.MAIL_DOMAIN_C // メールドメインコード（プルダウン選択時）
            };
            // 申込データに画像データを設定
            var imageData = {
                DOC_BASE_GAZO: subIf006Recv.FMT_DATA,
            };
        ...
        */
         ...
         * @return void
         */
        var errListOutput = function (param, order) {
