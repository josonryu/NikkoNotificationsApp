...
App.controller('homeMenuController', ['$scope', '$controller', 'logicCom', 'AppBizCom', 'AppBizCamera',
    function ($scope, $controller, logicCom, AppBizCom, AppBizCamera) {
        $controller('applicationCompController', { $scope: $scope });
        /** 画面IDの定義. */
        var PAGE_ID_MAIN = 'G1040-01';
        var PAGE_ID_NEXT = 'G1060-01';
        var PAGE_ID_MDAL = 'G1040-02';
        var PAGE_NM_NEXT = 'selectCustomer';
        /** ボタン名／名称 */
        ...
        hostNames.forEach((hostName) => {
            if (hostName.ipInformation && hostName.type === Windows.Networking.HostNameType.ipv4) {
                ipv4.push(hostName.displayName);
            }
        });
        /**
         * 初期化処理.
        ...
        var init = function () {
            ...
            //     logicCom.getSLSKeys(TYPE_EFORM, (result) => {
            //         // 成功
            ...
            // logicCom.getSLSKeys(TYPE_NOTIF, (result) => {
            //     // 成功
            //     $scope.slsKeys = $scope.slsKeys.concat(result);
            //     // SLS件数(申込種別：電子帳票)の取得
            //     getEformSlsKeys();
        ...
        */
        ...
        var sendApplyInfo = function (i) {
            ...
            $scope.$applyAsync();
            var key = $scope.slsKeys[i];
            setTimeout(() => {
                $scope.failedDataCnt++;
                return sendApplyInfo(++i);
            }, 1000);
            // $scope.sendApplyInfo(key, (data) => {
            //     // 成功
